...
import com.tw.async.util.ArrayDeque;
...
public class AsyncSocketMiddleware extends SimpleMiddleware {
    ...
    public AsyncSocketMiddleware(TwAsyncHttpClient client, String scheme, int port) {
        mClient = client;
        this.scheme = scheme;
        this.port = port;
    }
    ...
    public AsyncSocketMiddleware(TwAsyncHttpClient client) {
        this(client, "http", 80);
    }

    protected TwAsyncHttpClient mClient;
    ...
    String computeLookup(Uri uri, int port, String proxyHost, int proxyPort) {
        ...
        return uri.getScheme() + "//" + uri.getDecodedHost() + ":" + port + "?proxy=" + proxy;
    }
    ...
    public Cancellable getSocket(final GetSocketData data) {
        ...
        if (!connectAllAddresses || proxyHost != null || data.request.getProxyHost() != null) {
            ...
            if (data.request.getProxyHost() != null) {
                ...
                unresolvedHost = uri.getDecodedHost();
                unresolvedPort = port;
            }
            if (proxied) {
        ...
        Future<AsyncSocket> socket = mClient.getServer().getAllByName(uri.getDecodedHost())
