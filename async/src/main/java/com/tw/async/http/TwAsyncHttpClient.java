...
import com.tw.async.callback.ConnectCallback;
...
import com.tw.async.http.callback.HttpConnectCallback;
import com.tw.async.http.callback.RequestCallback;
import com.tw.async.parser.*;
import com.tw.async.stream.OutputStreamDataCallback;
import com.tw.async.util.TextUtils;
...
public class TwAsyncHttpClient {
    private static TwAsyncHttpClient mDefaultInstance;
    public static TwAsyncHttpClient getDefaultInstance() {
        if (mDefaultInstance == null)
            mDefaultInstance = new TwAsyncHttpClient(AsyncServer.getDefault());
    ...
    AsyncServer mServer;
    public TwAsyncHttpClient(AsyncServer server) {
        mServer = server;
...
//        }
    ...
    public static abstract class JSONObjectCallback extends RequestCallbackBase<ZSONObject> {
    }
    
    public static abstract class JSONArrayCallback extends RequestCallbackBase<ZSONArray> {
    }
    ...
    public Future<ZSONObject> executeJSONObject(AsyncHttpRequest req, final JSONObjectCallback callback) {
        return execute(req, new JSONObjectParser(), callback);
    }

    public Future<ZSONArray> executeJSONArray(AsyncHttpRequest req, final JSONArrayCallback callback) {
