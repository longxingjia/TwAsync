...
public class AsyncHttpServerRouter implements RouteMatcher {
    ...
    public static Asset getAssetStream(final Context context, String asset) {
        return getAssetStream(context.getResourceManager(), asset);
    }

    public static Asset getAssetStream(ResourceManager rm, String asset) {
        try {
            InputStream is = rm.getRawFileEntry(asset).openRawFile();
            ...
            for (String ext: extensions) {
                try {
                    InputStream is = rm.getRawFileEntry(asset + ext).openRawFile();
    ...
    static synchronized Manifest ensureManifest(Context context) {
        Future<Manifest> future = AppManifests.get(context.getBundleName());
        if (future != null)
        ...
        try {
            zip = new ZipFile(context.getBundleResourcePath());
            ...
            AppManifests.put(context.getBundleName(), result);
        }
    }
    ...
    static boolean isClientCached(Context context, AsyncHttpServerRequest request, AsyncHttpServerResponse response, String assetFileName) {
        ...
        try {
            ...
            Logger.w(AsyncHttpServerRouter.class.getSimpleName(), "Error getting ETag for apk asset", e);
            return false;
        }
    }
    ...
    public void directory(Context context, String regex, final String assetPath) {
        ...
        addAction(AsyncHttpGet.METHOD, regex, (request, response) -> {
            ...
            Asset pair = getAssetStream(rm, assetPath + path);
    ...
    abstract class AsyncHttpServerRequestImpl extends com.tw.async.http.server.AsyncHttpServerRequestImpl {
        Matcher matcher;
        @Override
