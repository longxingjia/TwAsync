...
import com.tw.async.http.server.AsyncHttpServerRequest;
import com.tw.async.http.server.AsyncHttpServerResponse;
...
public class WebSocketImpl implements WebSocket {
    ...
    private static String SHA1(String text) {
        try {
            ...
            return Base64Util.encodeToString(sha1hash, Base64Util.NO_WRAP);
        }
        catch (Exception ex) {
            return null;
        }
...
//            response.getHeaders().getHeaders().set("Access-Control-Allow-Origin", "http://" + origin);
    ...
    public static void addWebSocketUpgradeHeaders(AsyncHttpRequest req, String... protocols) {
        ...
        final String key = Base64Util.encodeToString(toByteArray(UUID.randomUUID()),Base64Util.NO_WRAP);
