...
import com.tw.async.callback.ListenCallback;
import com.tw.async.callback.ValueCallback;
import com.tw.async.http.Headers;
import com.tw.async.http.HttpUtil;
import com.tw.async.http.Multimap;
import com.tw.async.http.WebSocket;
    ...
    protected void onRequest(HttpServerRequestCallback callback, AsyncHttpServerRequest request, AsyncHttpServerResponse response) {
        if (callback != null) {
            try {
                ...
                Logger.e("AsyncHttpServer", "request callback raised uncaught exception. Catching versus crashing process", e);
                response.code(500);
                response.end();
            }
        }
