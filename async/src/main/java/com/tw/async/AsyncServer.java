...
//    public static void post(EventHandler handler, Runnable runnable) {
//        RunnableWrapper wrapper = new RunnableWrapper();
//
//        ThreadQueue threadQueue = ThreadQueue.getOrCreateThreadQueue(handler.getEventRunner());
...
//    }
    ...
    public Cancellable postDelayed(Runnable runnable, long delay) {
        ...
        synchronized (this) {
            ...
            long time;
            if (delay > 0)
                time = Clock.systemUTC().millis() + delay;
...
//        Log.i(LOGTAG, "****AsyncServer is shutting down.****");
        ...
        run(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Logger.e(LOGTAG, "Datagram error", e);
                    StreamUtility.closeQuietly(socket);
                }
            }
        });
    ...
    private void run() {
        ...
        try {
            ...
            Logger.i(LOGTAG, "Selector closed", e);
            try {
...
//        Logger.i(LOGTAG, "****AsyncServer is starting.****");
        ...
        while(true) {
            try {
                ...
                if (!(e.getCause() instanceof ClosedSelectorException))
                    Logger.i(LOGTAG, "Selector exception, shutting down", e);
...
//        Logger.i(LOGTAG, "****AsyncServer has shut down.****");
    }
...
//        Logger.i(LOGTAG, "Keys: " + selector.keys().size());
...
//                    Logger.i(LOGTAG, "Shutting down. keys: " + selector.keys().size() + " keepRunning: " + keepRunning);
                        return;
                    }
                }
                else {
    ...
    public void dump() {
        post(new Runnable() {
            ...
            public void run() {
                if (mSelector == null) {
                    Logger.i(LOGTAG, "Server dump not possible. No selector?");
                    return;
                }
                Logger.i(LOGTAG, "Key Count: " + mSelector.keys().size());

                for (SelectionKey key: mSelector.keys()) {
                    Logger.i(LOGTAG, "Key: " + key);
                }
            }
        });
    }
