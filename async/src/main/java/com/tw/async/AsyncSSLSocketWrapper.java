...
import com.tw.async.callback.*;
import com.tw.async.future.Cancellable;
import com.tw.async.future.SimpleCancellable;
import com.tw.async.http.SSLEngineSNIConfigurator;
import com.tw.async.util.Allocator;
import com.tw.async.util.Base64Util;
import com.tw.async.util.StreamUtility;
import com.tw.async.wrapper.AsyncSocketWrapper;
...
//        File keyPath = context.getFileStreamPath(subjectName + "-key.txt");
        ...
        try {
            ...
            X509EncodedKeySpec pub = new X509EncodedKeySpec(Base64Util.decode(keyParts[0], 0));
            PKCS8EncodedKeySpec priv = new PKCS8EncodedKeySpec(Base64Util.decode(keyParts[1], 0));

            cert = CertificateFactory.getInstance("X.509").generateCertificate(new ByteArrayInputStream(Base64Util.decode(keyParts[2], 0)));
            ...
            StreamUtility.writeFile(keyPath,
                    Base64Util.encodeToString(pair.getPublic().getEncoded(), Base64Util.NO_WRAP)
                            + "\n"
                            + Base64Util.encodeToString(pair.getPrivate().getEncoded(), Base64Util.NO_WRAP)
                            + "\n"
                            + Base64Util.encodeToString(cert.getEncoded(), Base64Util.NO_WRAP));
        }
    ...
    public static AsyncSSLServerSocket listenSecure(final Context context, final AsyncServer server, final String subjectName, final InetAddress host, final int port, final ListenCallback handler) {
        ...
        server.run(() -> {
            try {
                ...
                KeyPair pair = keyCert.f;
                Certificate cert = keyCert.s;
    ...
    public static AsyncSSLServerSocket listenSecure(AsyncServer server, String keyDer, String certDer, final InetAddress host, final int port, final ListenCallback handler) {
        return listenSecure(server, Base64Util.decode(keyDer, Base64Util.DEFAULT), Base64Util.decode(certDer, Base64Util.DEFAULT), host, port, handler);
    }
